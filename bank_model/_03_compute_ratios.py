...
DATA_PATH = "data/NHDATA_Mapped.xlsx"
...
def align_note_to_bctc(b: pd.DataFrame, n: pd.DataFrame,
    ...
    """
    Căn chỉnh NOTE theo đúng bộ khóa của BCTC để tránh lệch hàng.
    - Nếu NOTE trùng khóa: lấy dòng đầu (first). Bạn có thể đổi sang sum() nếu phù hợp.
    - Reindex NOTE theo index của BCTC.
    """
...
def build_base(bctc: pd.DataFrame, note: pd.DataFrame, freq: str):
    ...
    base = {
        ...
        "FLOW": {
            "NI": bctc["Lợi nhuận sau thuế thuộc về Ngân hàng mẹ"],
            "NII": bctc["Thu nhập lãi thuần"],
            "INT_INC": bctc["Thu nhập lãi và các khoản thu nhập tương tự"],
            "INT_EXP": bctc["Chi phí lãi và các chi phí tương tự"],
            "SERV_NET": bctc["Lãi thuần từ hoạt động dịch vụ"],
            "SERV_COST": bctc["Chi phí hoạt động dịch vụ"],
            "OTHER_NET": bctc["Lãi thuần từ hoạt động khác"],
            "INVEST_INC": bctc["Thu nhập từ góp vốn, mua cổ phần"],
            "PBT": bctc["Tổng lợi nhuận trước thuế"],
            "OPEX": bctc["Chi phí hoạt động"],
            "TOT_OP_INC": bctc["Tổng thu nhập hoạt động"],
        },
    }
    return base
...
def compute_master(base: dict) -> pd.DataFrame:
    ...
    bctc_need_numeric = [
        "Thu nhập từ hoạt động dịch vụ", "3. Tiền gửi của khách hàng",
    ...
    m["Tỷ lệ chi phí dịch vụ/ thu nhập dịch vụ"] = F["SERV_COST"] / b["Thu nhập từ hoạt động dịch vụ"]
    ...
    m["DPRRTD/CV Khách hàng và CK đầu tư"] = b["Chi phí dự phòng rủi ro tín dụng"] / (
        b["Thu nhập từ góp vốn, mua cổ phần"]
        + b["Lợi nhuận thuần từ hoạt động kinh doanh trước chi phí dự phòng rủi ro tín dụng"]
    ...
    if k == 1:
        ...
        m["Annual Interest Income Growth"]   = b.groupby("Ticker")["Thu nhập lãi và các khoản thu nhập tương tự"].pct_change(fill_method=None)
        m["Annual Net Income Growth"]        = b.groupby("Ticker")["Lợi nhuận sau thuế thuộc về Ngân hàng mẹ"].pct_change(fill_method=None)
        ...
        m["QYoY Interest Income Growth"]    = gq["Thu nhập lãi và các khoản thu nhập tương tự"].pct_change(fill_method=None)
        m["QYoY Net Income Growth"]         = gq["Lợi nhuận sau thuế thuộc về Ngân hàng mẹ"].pct_change(fill_method=None)
...
with pd.ExcelWriter("data/Ratios.xlsx", engine="openpyxl") as w:
